...
// ######################## СЛИЯНИЕ ВЕКТОРОВ ########################
template<typename T>
inline void ConsoleUI<T>::VectorMerge()
{
	ChoseVector();
	T elem;
	vector<T> Merge;
	char exit;
	cout << "Заполните новый вектор:\n";
	for (int i = 1; true; i++)
	{
		cout << i << ". Введите значение: ";
		cin >> elem;
		Merge.push_back(elem);
		cout << "Желаете ли вы продолжить? (\"-\" для выхода) : ";
		cin >> exit;
		if (exit == '-')
			break;
	}
	std::copy(Merge.begin(), Merge.end(), back_inserter(Vector));
	cout << "\nНовый вектор успешно слился со основным.\n";
	OperationPass();
}
// ###################### СОРТИРОВКА ВЕКТОРА ########################
template<typename T>
inline void ConsoleUI<T>::VectorSort(int ch)
{
	if (NoVectorData())
		return;
	...
	{
	case 1:
		sort(Vector.begin(), Vector.end(), minmax);
		cout << endl;
		OperationPass();
		break;

	case 2:
		sort(Vector.begin(), Vector.end(), maxmin);
		cout << endl;
		OperationPass();
		break;
